---
name: qr-simple-ui
description: Use when styling, reviewing, or visually verifying qr-simple's admin UI (Blazor pages under src/QrSimple.Api/Components/Pages, rendered at /app) or either public page (src/QrSimple.Api/ScanPage.cs at /e/{id}, and src/QrSimple.Api/InspectionsPage.cs at /e/{id}/inspections) — covers the shared design tokens all three surfaces must reuse and the Playwright MCP loop for confirming a change actually rendered.
---
...
4. `browser_resize` to a phone width (~390×844) and screenshot again — every admin page and both public pages must stay usable there; the `@media` blocks in `app.css`/`ScanPage.cs`/`InspectionsPage.cs` are where that's enforced.
