...
- `POST`/`GET /users` are both gated now: `POST /users` allows unauthenticated self-registration only while no Admin exists yet (bootstrap case) and otherwise requires an authenticated Admin; `GET /users` requires an authenticated Admin unconditionally (see `UserAuthorizationTests.cs` / `UserBootstrapTests.cs`). The admin UI's `/app/not-authorized` page still points a newly-signed-in user at the manual `POST /users` step for the pre-bootstrap case. There's no Users-management UI yet — adding a user means an Admin calling `POST /users` directly (e.g. via `fetch` in the browser console while signed in).
...
**Next natural slices** if picking this back up: a Users-management UI (`POST`/`GET /users` are gated but have no admin-UI page), wiring real Google credentials for a live deployment (the wizard above), a category-management UI, or anything from the "explicit non-goals" list above if priorities have changed.
