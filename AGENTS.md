...
- `GET /users` is still ungated. `POST /users` is no longer wide open: it allows unauthenticated self-registration only while no Admin exists yet (bootstrap case), and requires the caller to already be an authenticated Admin once one exists (see `UserAuthorizationTests.cs` / `UserBootstrapTests.cs`). The admin UI's `/app/not-authorized` page still points a newly-signed-in user at the manual `POST /users` step for the pre-bootstrap case.
...
**Known bugs (not deliberate, not yet fixed as of 2026-08-14)**:
- `POST /logout` (`Program.cs`) redirects to `Results.Redirect("/")` after signing out, but no route is mapped at `/` — only `/app` and its children, plus `/login`. Clicking "Sign out" lands on a raw browser 404 instead of a real page. Sign-out itself works correctly (the cookie is cleared); only the redirect target is wrong. Fix is a one-line change to `Results.Redirect("/app")`.

**Next natural slices** if picking this back up: wiring real Google credentials for a live deployment (the wizard above), a category-management UI, fixing the `/logout` redirect above, gating `GET /users`, or anything from the "explicit non-goals" list above if priorities have changed.
