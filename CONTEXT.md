...
**Document**:
A labeled file (e.g. "User manual", "Maintenance instruction", "Safety Data Sheet", or the equipment photo) attached to a piece of Equipment for the "more details" flow. Uploaded directly and stored as bytes in Postgres — v1 hosts the file itself rather than linking out to wherever it already lives. The data model keeps room for a URL-based link mode alongside the upload fields for a possible future phase, but nothing in the app creates a URL-based Document today.
_Note_: **"Document" is the entity name, not what the admin UI calls it.** As of 2026-09-01 the generic list on `/app/equipment/{id}` is headed **"Manuals"** (upload control "Manual file" / "Add manual"). It was "Documents" until 2026-08-31 and "User Manual" for one day after that; the plural replaced the singular because the section holds the user manual *and* the maintenance manual *and* anything else of that kind, which the singular heading denied. All three were copy renames only — the entity, the `Documents` table, `DocumentCatalog` and the `/equipment/{id}/documents` routes keep the `Document` name, because the equipment photo and the OEM QA/QC report are stored as `Document` rows too and "manual" would be wrong for them. A Safety Data Sheet filed there appears under the "Manuals" heading.
