...
4. Use the Playwright MCP browser tools to navigate to `https://localhost:7040/app`.
   ...
   - This should land on the app's own login page at `/login` — ICS logo, "Equipment Registry", a "Continue with Google" button. As of 2026-08-31 it no longer jumps straight through to Google; click that button to get there, and expect Google's **account picker** (the challenge sends `prompt=select_account`). If you instead see `invalid_client`, `access_blocked`, or any error *before* reaching Google's normal email/account screen, stop and report that precisely — it would mean the publish broke something, contrary to what was already observed.
