...
Status: Implemented, pending live browser verification (see 2026-08-29 Log entry — code/migration/docs complete, 112/112 tests green, but the Playwright live-verification checklist below is blocked this session by the host browser stack being down; hand off to a fresh session per the Log)
...
- [ ] **BLOCKED, not verified this session** — Live in a browser via the `qr-simple-ui` skill's Playwright loop:
  - [ ] Upload a PDF as **Operator** at `/app/equipment/{id}/inspections` — note, kind, and a backdated inspection date all persist and render.
  - [ ] The `InputFile` picker clears after a successful upload (the `@key` bump actually works).
  - [ ] Operator edits their own record's note/date successfully; the last-edited line appears admin-side.
  - [ ] Operator sees **no Delete control**; an Admin does, and it works behind the confirm dialog.
  - [ ] `/e/{id}` shows the new panel with the right count, styled as a sibling of the existing document links.
  - [ ] `/e/{id}/inspections` in a **logged-out** context (incognito or cleared cookies) — list, PDF open, and back link all work with no login, and **no email address appears anywhere in the page source**. This is decisions 10 and 11 and cannot be verified while signed in.
  - [ ] Opening a PDF on a phone-width viewport **displays inline** rather than triggering a download, and the saved name is the generated one (decision 19).
  - [ ] Seed one inspection older than 6 months: it sits inside the collapsed `<details>`, the summary count is right, and it expands.
  - [ ] Seed equipment whose only inspections are all older than 6 months: **3 still show as recent** (decision 17).
  - [ ] Retire an equipment: its inspections page still loads and lists history, and the upload form is gone.
  - [ ] Both pages at ~390×844 phone width.
  - [ ] `browser_console_messages` clean.
  - [ ] `getComputedStyle(document.documentElement).getPropertyValue("--brand-primary")` resolves on the new page — confirms `tokens.css` actually loaded rather than the page silently falling back to unstyled defaults.
- [x] Docs updated per "Docs to update", including the `qr-simple-ui` skill.
- [ ] Status flipped to `Implemented` here and in `docs/plans/README.md`'s index row — **not flipped**; see 2026-08-29 implementation Log entry below. Set to `Implemented, pending live browser verification` instead.
...
**Not verified this session — live browser step blocked**: Playwright MCP and the `qr_simple` project MCP both show `ConnectionRefused` for this entire session (checked via `ToolSearch` — ASK before assuming a retry would help: it won't, `docs/local-browser-testing.md`'s "Claude Code shows playwright/qr_simple as failed to connect" section confirms MCP servers resolve once at session start with no retry). Beyond that, the whole host stack was confirmed down by direct `curl`, not just unresolved by this session: `127.0.0.1:9222` (Chrome CDP), `127.0.0.1:8931` (Playwright MCP), `127.0.0.1:5078` (API), and `127.0.0.1:5432` (dev Postgres) all refused connections outright when checked from the devcontainer. So none of the "Live in a browser" checklist items below were exercised — everything on them is implemented and covered by the HTTP-level integration test suite (which does exercise the real routes, role gating, and rendered HTML end-to-end, just not real-browser layout/console/viewport), but not confirmed visually. **Next step for whoever picks this up**: run `./scripts/start-chrome-for-playwright.sh` from a Fedora host terminal, start the API in a devcontainer terminal (`--launch-profile https`), then hand the live-verification checklist to a *fresh* Claude Code/Codex session started after the stack is confirmed up (per that doc's copy/paste prompt) — not this session. Status set to `Implemented, pending live browser verification` rather than `Implemented` until that step actually runs.
