...
- [ ] `dotnet build` clean; full suite green in the devcontainer (a scoped `--filter` rerun is acceptable evidence for inotify flakiness, per AGENTS.md).
- [ ] `dotnet ef migrations add AddInspections` generated, `Up`/`Down` reviewed, applies cleanly on a database already carrying the three existing migrations.
- [ ] `TimeZoneInfo.FindSystemTimeZoneById("Asia/Ulaanbaatar")` resolves at runtime in the devcontainer (it's confirmed present in the production image; confirm the dev environment too rather than discovering it in CI).
- [ ] Live in a browser via the `qr-simple-ui` skill's Playwright loop:
...
- [ ] Docs updated per "Docs to update", including the `qr-simple-ui` skill.
- [ ] Status flipped to `Implemented` here and in `docs/plans/README.md`'s index row.
