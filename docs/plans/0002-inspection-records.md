...
2026-08-31: Correction to this plan's Design section, for anyone reading it as a map of the code. Line ~267 says to "link to this page from `EquipmentDetail.razor`'s Documents section" — as built, the link lives in its **own** `<h2>Rebuild history</h2>` section on that page, not inside the generic document list. Decision 21 (a separate page rather than a section bolted onto `EquipmentDetail.razor`) is unaffected and was honoured; only the sketch's placement of the entry link differs. Separately, the generic list that sketch refers to is now headed "Manuals" (see `docs/plans/0001-document-file-upload.md`'s 2026-08-31 and 2026-09-01 log entries — it was briefly "User Manual" in between) — UI copy renames that touched nothing this plan depends on.

2026-09-01: This plan's rebuild form is affected by an app-wide styling change made for the equipment detail page — recorded here so the Design sketch is not read as current. Every `<label>` in the admin UI is now a two-column grid on `--label-col`/`--label-gap`, so "Rebuild date", "Note" and "PDF file (optional)" put their controls in one aligned column rather than each starting after its own label text. The alignment is `baseline`, chosen specifically so the Note's label sits level with the first line of its textarea instead of centring against a 92px-tall box. Decision 4 (PDF-only rebuild attachments), decision 21 (a separate page) and the plan's field set are all unaffected — this is layout only, and the two Note textareas' placeholder text added 2026-08-31 is unchanged.
