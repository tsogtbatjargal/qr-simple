...
| [0002](0002-inspection-records.md) | Periodic inspection records for Equipment | Implemented, pending live browser verification | New `Inspection` entity (dated, noted, attributed PDF) uploaded per Equipment by Admin/Operator, browsable publicly from the scan page at `/e/{id}/inspections` with a 6-month recent window and older records collapsed. |
